...
class Aipex_Podcast_Migration {
    ...
    public static function migrate_ova_episodes($dry,&$log,$host_map){
        ...
        foreach($episodes as $e){
            ...
            Aipex_Podcast_Utils::update_field('publish_date', date('Y-m-d', strtotime($e->post_date)), $pid);
            ...
            if($presenters) Aipex_Podcast_Utils::update_field('presenters', array_values(array_unique($presenters)), $pid);
    ...
    public static function normalise_v2_existing($dry,&$log){
        ...
        foreach($episodes as $id){
            ...
            foreach($episode_fields as $field){
                ...
                $alias_value=Aipex_Podcast_Utils::field($field,$id,null);
                if(($current==='' || $current===[] || $current===null) && $alias_value!==null && $alias_value!=='' && $alias_value!==[]){
                    $changes[]=$field.' copied';
                    if(!$dry) Aipex_Podcast_Utils::update_field($field,$alias_value,$id);
                }
            }
            $src=Aipex_Podcast_Utils::field('audio_source_type',$id);
            if(!$src){
                if(Aipex_Podcast_Utils::field('dropbox_url',$id)) $src='dropbox';
                elseif(Aipex_Podcast_Utils::field('soundcloud_url',$id)) $src='soundcloud';
                elseif(Aipex_Podcast_Utils::field('audio_file',$id)) $src='upload';
                if($src){ $changes[]='audio_source_type='.$src; if(!$dry) Aipex_Podcast_Utils::update_field('audio_source_type',$src,$id); }
            }
            if(!Aipex_Podcast_Utils::field('publish_date',$id)){ $date=get_the_date('Y-m-d',$id); $changes[]='publish_date='.$date; if(!$dry) Aipex_Podcast_Utils::update_field('publish_date',$date,$id); }
        ...
        foreach($series as $id){
            ...
            foreach(['series_overview','series_main_points','series_episode_summaries','rss_url','spotify_url','apple_url','youtube_url','amazon_url','pocketcasts_url','website'] as $field){
                $current=get_post_meta($id,$field,true); $alias_value=Aipex_Podcast_Utils::field($field,$id,null);
                if(($current==='' || $current===[] || $current===null) && $alias_value!==null && $alias_value!=='' && $alias_value!==[]){ $changes[]=$field.' copied'; if(!$dry) Aipex_Podcast_Utils::update_field($field,$alias_value,$id); }
            }
    ...
    public static function copy_first_meta($from,$to,$keys,$dest){
        foreach($keys as $key){
            ...
            if($v!=='' && $v!==null && $v!==[]){ Aipex_Podcast_Utils::update_field($dest,$v,$to); return true; }
        }
        return false;
    }
}
