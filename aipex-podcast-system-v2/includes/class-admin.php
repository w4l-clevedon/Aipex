...
class Aipex_Podcast_Admin {
    ...
    public static function handle_actions(){
        ...
        if(isset($_POST['aipex_apply_default_sponsor'])){ check_admin_referer('aipex_tools'); $msg=self::apply_default_sponsor((int)($_POST['default_sponsor_id']??20740), !empty($_POST['replace_existing_sponsors'])); self::notice($msg); }
        if(isset($_POST['aipex_remove_default_sponsor'])){ check_admin_referer('aipex_tools'); $msg=self::remove_default_sponsor((int)($_POST['default_sponsor_id']??20740)); self::notice($msg); }
    }
    ...
    public static function tools(){
        ...
        echo '<p><label>Sponsor ID <input type="number" name="default_sponsor_id" value="20740" min="1" style="width:120px"></label></p>';
    ...
    public static function sync_dates_durations(){
        ...
        foreach($q->posts as $id){ Aipex_Podcast_Utils::update_field('publish_date', get_the_date('Y-m-d',$id), $id); $file=Aipex_Podcast_Utils::field('audio_file',$id); if(is_numeric($file)){ $m=wp_get_attachment_metadata((int)$file); if(!empty($m['length_formatted'])) Aipex_Podcast_Utils::update_field('duration',$m['length_formatted'],$id); } $n++; }
    ...
    public static function match_txt_target($name,$text,$type){
        ...
        $best=null; $score=0;
        foreach($candidates as $p){ $s=Aipex_Podcast_Utils::match_score($needle,$p->post_title); if($s>$score){ $score=$s; $best=$p; } }
    ...
    public static function apply_txt_to_target($post_id,$type,$raw,$name){
        ...
        if($type==='series_overview'){
            ...
            Aipex_Podcast_Utils::update_field('series_overview',$parsed['summary'],$post_id);
            if($parsed['points']) Aipex_Podcast_Utils::update_field('series_main_points', array_map(fn($p)=>['point'=>$p],$parsed['points']), $post_id);
            if($parsed['episode_summaries']) Aipex_Podcast_Utils::update_field('series_episode_summaries', self::link_series_episode_summaries($parsed['episode_summaries'],$post_id), $post_id);
            return;
        }
        if($type==='episode_summary'){
            $parsed=self::parse_summary_like_text($raw, false, $name);
            Aipex_Podcast_Utils::update_field('episode_summary',$parsed['summary'],$post_id);
            if($parsed['points']) Aipex_Podcast_Utils::update_field('main_points', array_map(fn($p)=>['point'=>$p],$parsed['points']), $post_id);
            if($parsed['hashtags']) wp_set_post_terms($post_id,$parsed['hashtags'],'post_tag',true);
            return;
        }
        Aipex_Podcast_Utils::update_field('transcript',$raw,$post_id);
    }
    ...
    public static function link_series_episode_summaries($rows,$series_id){
        ...
        foreach($rows as $r){
            ...
            $episode_id=0; $q=Aipex_Podcast_Utils::query_episodes(['series_id'=>$series_id,'post_status'=>'any','posts_per_page'=>-1]);
            foreach($q->posts as $ep){ if(Aipex_Podcast_Utils::match_score($name,$ep->post_title)>=88){ $episode_id=$ep->ID; break; } }
    ...
    public static function replace_bad_audio_urls(){
        ...
        foreach($q->posts as $id){
            $url=Aipex_Podcast_Utils::audio_url($id); $drop=Aipex_Podcast_Utils::field('dropbox_url',$id);
            ...
            foreach($index as $item){ $s=max(Aipex_Podcast_Utils::match_score($base,$item['name']??''), Aipex_Podcast_Utils::match_score(get_the_title($id),$item['name']??'')); if($s>$score){$score=$s;$best=$item;} }
    ...
    public static function scan_duplicates(){
        ...
        foreach($eps as $ep){
            $title_key=Aipex_Podcast_Utils::normalize($ep->post_title);
            $audio=Aipex_Podcast_Utils::audio_url($ep->ID); $audio_key=$audio ? md5(strtolower(basename(parse_url($audio,PHP_URL_PATH) ?: $audio))) : '';
    ...
    public static function get_show_sponsors($series_id){
        $value=Aipex_Podcast_Utils::field('series_sponsors',$series_id);
        if (empty($value)) $value=Aipex_Podcast_Utils::field('sponsors',$series_id);
        return self::normalise_relationship_ids($value);
    }
    public static function set_show_sponsors($series_id,$ids){
        $ids=array_values(array_unique(array_map('intval',(array)$ids)));
        Aipex_Podcast_Utils::update_field('series_sponsors',$ids,$series_id);
        // Also mirror to sponsors for compatibility with any older V2 builds/shortcodes.
        Aipex_Podcast_Utils::update_field('sponsors',$ids,$series_id);
    }
    public static function apply_default_sponsor($sponsor_id=20740,$replace=false){
    ...
    public static function remove_default_sponsor($sponsor_id=20740){
        $series=get_posts(['post_type'=>'aipex_series','post_status'=>'any','posts_per_page'=>-1,'fields'=>'ids']);
        $updated=0;
        foreach($series as $series_id){
            $current=self::get_show_sponsors($series_id);
