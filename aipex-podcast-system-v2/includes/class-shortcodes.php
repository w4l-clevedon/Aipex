...
class Aipex_Podcast_Shortcodes {
    ...
    public static function aipex_floating_player($atts=[]){
        ...
        if ($atts['context'] === 'auto') {
            $series_id = Aipex_Podcast_Utils::current_context('aipex_series');
            $presenter_id = Aipex_Podcast_Utils::current_context('aipex_presenter');
        ...
        while ($q->have_posts()) { $q->the_post();
            ...
            $tracks[] = [
                ...
                'duration'=>Aipex_Podcast_Utils::field('duration', get_the_ID(), '')
            ];
        }
        wp_reset_postdata();
        if (!$tracks) return '';
    ...
    public static function grid($args=[]){
        ...
        $q = Aipex_Podcast_Utils::query_episodes([
    ...
    public static function episode_card($id){
        ...
        return '<article class="aipex-card aipex-episode-card">'.
            ...
            Aipex_Podcast_Utils::button(get_permalink($id),'Listen To '.get_the_title($id)).
            '</div></article>';
    }
    ...
    public static function aipex_series_podcasts($atts=[]){ $id=Aipex_Podcast_Utils::current_context('aipex_series'); $a=shortcode_atts(['limit'=>12],$atts); $a['series_id']=$id; return self::grid($a); }
    public static function aipex_show_podcasts($atts=[]){ return self::aipex_series_podcasts($atts); }
    public static function aipex_presenter_podcasts($atts=[]){ $id=Aipex_Podcast_Utils::current_context('aipex_presenter'); $a=shortcode_atts(['limit'=>12],$atts); $a['presenter_id']=$id; return self::grid($a); }
    ...
    public static function profile_card($id, $label){
        ...
        if (!$excerpt) $excerpt = Aipex_Podcast_Utils::field($label==='show'?'series_overview':'presenter_about',$id,'');
        return '<article class="aipex-card aipex-profile-card aipex-'.$label.'-card">'.
            ...
            Aipex_Podcast_Utils::button(get_permalink($id),'Learn More About '.$name).
            '</div></article>';
    }
    ...
    public static function sponsor_card($id,$message='',$url=''){
        ...
        $name=get_the_title($id);
        $img=get_the_post_thumbnail($id,'medium_large');
        $about=Aipex_Podcast_Utils::field('aipex_sponsor_about',$id,get_the_excerpt($id));
        $website=$url ?: Aipex_Podcast_Utils::field('website',$id);
        ...
        if($website) $out.=Aipex_Podcast_Utils::button($website,'Visit '.$name);
        $out.='</div></article>';
        return $out;
    }
    ...
    public static function aipex_sponsors($atts=[]){
        ...
        $rows=Aipex_Podcast_Utils::field('sponsors',$post_id,[]);
        if(is_array($rows)){
            foreach($rows as $row){
        ...
        if(!$cards && get_post_type($post_id)==='aipex_podcast'){
            $series=Aipex_Podcast_Utils::ids('series',$post_id);
            foreach($series as $sid){
                $srows=Aipex_Podcast_Utils::field('aipex_series_sponsors',$sid,[]);
    ...
    public static function ajax_grid_load_more(){
        ...
        if ($kind === 'series' || $kind === 'presenters') {
            ...
            $q = Aipex_Podcast_Utils::query_episodes(['posts_per_page'=>$limit,'paged'=>$page,'series_id'=>(int)($context['series_id']??0),'presenter_id'=>(int)($context['presenter_id']??0)]);
