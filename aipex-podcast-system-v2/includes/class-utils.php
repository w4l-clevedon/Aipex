...
class Aipex_Podcast_Utils {
    public static function aliases($key){
        $map = [
            ...
            'series' => ['aipex_series'],
            'presenters' => ['aipex_presenters','ovau_host_id'],
            'guests' => ['aipex_guests'],
    ...
    public static function field($key,$post_id=null,$default=''){
        ...
        foreach ($keys as $k) {
            if (function_exists('get_field')) { $v = get_field($k,$post_id); if ($v!==null && $v!==false && $v!=='' && $v!==[]) return $v; }
            $v = get_post_meta($post_id,$k,true); if ($v!=='' && $v!==null && $v!==[]) return $v;
        }
        return $default;
    }
    ...
    public static function ids($key,$post_id=null){
        ...
        if (is_array($v)) return array_values(array_unique(array_filter(array_map(function($x){return is_object($x)&&isset($x->ID)?(int)$x:(is_numeric($x)?(int)$x:0);},$v))));
        return [];
    }
    ...
    public static function current_context($type){
        ...
        $qid = get_queried_object_id(); if ($qid && get_post_type($qid)===$type) return $qid;
        global $post; return ($post && $post->post_type===$type) ? $post->ID : 0;
    }
    public static function relationship_meta_query_for_id($key,$id){
        $id=(int)$id; $keys=array_merge([$key], self::aliases($key));
        $or=['relation'=>'OR'];
        foreach($keys as $k){
