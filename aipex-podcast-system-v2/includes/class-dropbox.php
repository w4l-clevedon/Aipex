...
class Aipex_Podcast_Dropbox {
    ...
    public static function page(){
        ...
        $app_secret=get_option('aipex_dropbox_app_secret','');
        $manual_token=get_option('aipex_dropbox_token','');
        $refresh=get_option('aipex_dropbox_refresh_token','');
    ...
    public static function handle_actions(){
        ...
        update_option('aipex_dropbox_app_secret', sanitize_text_field(wp_unslash($_POST['app_secret']??'')));
        update_option('aipex_dropbox_token', sanitize_text_field(wp_unslash($_POST['token']??'')));
    ...
    public static function exchange_code($code){
        $key=get_option('aipex_dropbox_app_key',''); $secret=get_option('aipex_dropbox_app_secret',''); if(!$key || !$secret) return 'Missing Dropbox App Key or App Secret.';
        ...
        update_option('aipex_dropbox_access_token',sanitize_text_field($body['access_token'])); update_option('aipex_dropbox_access_expires',time()+intval($body['expires_in']??14400)-120); if(!empty($body['refresh_token'])) update_option('aipex_dropbox_refresh_token',sanitize_text_field($body['refresh_token'])); return 'Dropbox connected successfully.';
    }

    public static function get_access_token(){
        $manual=get_option('aipex_dropbox_token',''); $refresh=get_option('aipex_dropbox_refresh_token',''); $access=get_option('aipex_dropbox_access_token',''); $expires=(int)get_option('aipex_dropbox_access_expires',0);
        if($refresh){ if($access && $expires>time()+60) return $access; $key=get_option('aipex_dropbox_app_key',''); $secret=get_option('aipex_dropbox_app_secret',''); $res=wp_remote_post('https://api.dropboxapi.com/oauth2/token',['timeout'=>45,'body'=>['grant_type'=>'refresh_token','refresh_token'=>$refresh,'client_id'=>$key,'client_secret'=>$secret]]); if(!is_wp_error($res)){ $body=json_decode(wp_remote_retrieve_body($res),true); if(!empty($body['access_token'])){ update_option('aipex_dropbox_access_token',sanitize_text_field($body['access_token'])); update_option('aipex_dropbox_access_expires',time()+intval($body['expires_in']??14400)-120); return $body['access_token']; } } }
        return $manual;
    }
    ...
    public static function scan(){
        ...
        foreach($files as $f){ $best=null; $score=0; foreach($episodes as $ep){ $s=Aipex_Podcast_Utils::match_score($f['name'],$ep->post_title); if($s>$score){$score=$s;$best=$ep;} }
    ...
    public static function attach_dropbox_url($episode_id,$url){ Aipex_Podcast_Utils::update_field('audio_source_type','dropbox',$episode_id); Aipex_Podcast_Utils::update_field('dropbox_url',Aipex_Podcast_Utils::dropbox_direct($url),$episode_id); }
    ...
    public static function shared_url_result($path){
        ...
        if(!is_wp_error($res)){ $raw=wp_remote_retrieve_body($res); $body=json_decode($raw,true); if(!empty($body['url'])) return ['url'=>Aipex_Podcast_Utils::dropbox_direct($body['url']),'error'=>'']; $err=self::dropbox_error($raw); }
        else $err=$res->get_error_message();
        $res2=self::api('sharing/list_shared_links',['path'=>$path,'direct_only'=>true]);
        if(!is_wp_error($res2)){ $raw2=wp_remote_retrieve_body($res2); $body=json_decode($raw2,true); if(!empty($body['links'][0]['url'])) return ['url'=>Aipex_Podcast_Utils::dropbox_direct($body['links'][0]['url']),'error'=>'']; if(empty($err)) $err=self::dropbox_error($raw2); }
    ...
    public static function process_scan_entries($entries, &$log){
        ...
        foreach($entries as $e){
            ...
            foreach($episodes as $ep){
                $s = Aipex_Podcast_Utils::match_score($name, $ep->post_title);
